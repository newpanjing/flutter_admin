...
class _OverflowPageState extends State<OverflowPage> {
  final List<Map<String, dynamic>> _overflowRecords = [
    {
      'id': 'OF001',
      ...
      'unitPrice': 12999.00,
      'totalValue': 38997.00,
      'overflowDate': '2024-01-15',
      'operator': '张三',
      'reason': '供应商多发货，未及时更新系统',
      'status': '已确认',
      'approver': '李经理',
    ...
    },
    {
      ...
      'unitPrice': 89.00,
      'totalValue': 445.00,
      'overflowDate': '2024-01-18',
      'operator': '王五',
      'reason': '盘点发现多出库存，可能是退货未登记',
      'status': '待审核',
      'approver': '',
      'approveDate': '',
    },
    {
      ...
      'unitPrice': 899.00,
      'totalValue': 1798.00,
      'overflowDate': '2024-01-20',
      'operator': '李四',
      'reason': '客户取消订单后商品退回，系统未及时更新',
      'status': '已确认',
      'approver': '张经理',
  ...
  void initState() {
    super.initState();
    _filteredRecords = List.from(_overflowRecords);
  }

  void _filterRecords() {
    setState(() {
      _filteredRecords = _overflowRecords.where((record) {
      ...
      body: Column(
        children: [
                ...
                Row(
                  children: [
                    ...
                    ModernButton(
                      text: '新增溢出',
                      icon: Icons.add,
                      onPressed: () => _showAddOverflowDialog(),
                    ),
                  ],
                ),
          ...
          Container(
            ...
            child: Row(
              children: [
                _buildStatCard('本月溢出', '3', '项', const Color(0xFF4CAF50)),
                const SizedBox(width: 16),
                _buildStatCard('盈余价值', '41,240', '元', Colors.blue),
                const SizedBox(width: 16),
              ...
              child: Column(
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        _buildTableHeader('溢出编号', flex: 2),
                        _buildTableHeader('商品信息', flex: 3),
                        _buildTableHeader('仓库', flex: 1),
                        _buildTableHeader('预期/实际', flex: 2),
                        _buildTableHeader('溢出数量', flex: 1),
                        _buildTableHeader('盈余价值', flex: 2),
                        _buildTableHeader('溢出日期', flex: 2),
        ...
        children: [
          ...
          Expanded(
            ...
            child: Row(
              children: [
                IconButton(
                  onPressed: () => _showOverflowDetail(record),
                  icon: const Icon(
                ...
                if (record['status'] == '待审核') ...[
                  IconButton(
                    onPressed: () => _approveOverflow(record),
                    icon: const Icon(
                  ...
                  IconButton(
                    onPressed: () => _rejectOverflow(record),
                    icon: const Icon(
                  ...
                  IconButton(
                    onPressed: () => _deleteOverflow(record),
                    icon: const Icon(
  ...
  void _showOverflowDetail(Map<String, dynamic> record) {
    showDialog(
      context: context,
      builder: (context) => ModernDialog(
        title: '溢出详情',
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildDetailRow('溢出编号', record['id']),
            ...
            _buildDetailRow('预期数量', record['expectedQuantity'].toString()),
            _buildDetailRow('实际数量', record['actualQuantity'].toString()),
            _buildDetailRow('溢出数量', '+${record['overflowQuantity']}'),
            _buildDetailRow('单价', '¥${record['unitPrice'].toStringAsFixed(2)}'),
            _buildDetailRow('盈余价值', '¥${record['totalValue'].toStringAsFixed(2)}'),
            _buildDetailRow('溢出日期', record['overflowDate']),
            _buildDetailRow('操作员', record['operator']),
            _buildDetailRow('溢出原因', record['reason']),
  ...
  void _approveOverflow(Map<String, dynamic> record) {
    showDialog(
      context: context,
      builder: (context) => ModernDialog(
        title: '审核通过',
  ...
  void _rejectOverflow(Map<String, dynamic> record) {
    showDialog(
      context: context,
      builder: (context) => ModernDialog(
        title: '审核拒绝',
  ...
  void _deleteOverflow(Map<String, dynamic> record) {
    showDialog(
      context: context,
      builder: (context) => ModernDialog(
        title: '确认删除',
        ...
        actions: [
          ...
          ModernButton(
            ...
            onPressed: () {
              Navigator.of(context).pop();
              setState(() {
                _overflowRecords.removeWhere((r) => r['id'] == record['id']);
                _filterRecords();
              });
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('溢出记录已删除'),
                  backgroundColor: Colors.red,
                ),
              );
            },
